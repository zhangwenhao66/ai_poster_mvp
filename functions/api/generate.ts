...
/** 方舟生图可能较慢；本地 miniflare 偶发在读响应体阶段断连，做有限次重试并避免压缩解压链路问题。 */
async function forwardToArk(apiKey: string, jsonBody: string): Promise<Response> {
    ...
    try {
      const upstream = await fetch(ARK_URL, {
        ...
        headers: {
          ...
          // 部分本地/代理环境对 br 解压不稳定；要求不压缩以降低读体失败概率（体积略增通常可接受）
  ...
  const hint =
    isTransientNetError(detail) || detail.includes("Network")
      ? "本地 wrangler 出站不稳定（代理/安全软件/生图耗时长导致读响应中断）较常见。可尝试：关闭系统代理；设置 NO_PROXY；换手机热点；或直接部署到 Cloudflare Pages 由边缘网络访问方舟。"
      : "";
  return jsonResponse(
    {
      error: {
...
async function handlePost(context: { request: Request; env: { ARK_API_KEY: string } }) {
  const apiKey = context.env.ARK_API_KEY;
  if (!apiKey) {
  ...
  const payload: ArkImagePayload = {
    model,
    prompt: prompt.trim(),
    size: typeof incoming.size === "string" ? incoming.size : "2K",
...
export const onRequest: PagesFunction<{ ARK_API_KEY: string }> = async (context) => {
